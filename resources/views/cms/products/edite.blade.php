                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="name" value="{{ $product->name }}"
                                        name="name" placeholder="{{ __('cms.name') }}">
                                </div>
                                <div class="form-group">
                                    <label for="price">{{ __('cms.price') }}</label>
                                    <input type="number" class="form-control" id="price" name="price"
                                        value="{{ $product->price }}" placeholder="{{ __('cms.price') }}">
                                </div>
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <input id="rate" type="hidden" name="rate" value="{{ $product->rate }}">
                                    <div id="rateYo"></div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select class="form-control subCategories" style="width: 100%;" id="category_id"
                                        ...
                                        @foreach ($subCategories as $subCategory)
                                            <option value="{{ $subCategory->id }}"
                                                @if ($product->category_id == $subCategory->id) selected @endif>{{ $subCategory->title }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                        ...
                                        <select id="nutrients-select" class="select2 select2-hidden-accessible"
                                            ...
                                            @foreach ($nutrients as $nutrient)
                                                <option value="{{ $nutrient->id }}"
                                                    @if ($selected_nutrient->pluck('id')->contains($nutrient->id)) selected @endif>{{ $nutrient->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            $("#rateYo").rateYo({
                rating: {{$product->rate}}, // Initial rating value
